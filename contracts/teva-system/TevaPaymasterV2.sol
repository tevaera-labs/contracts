...
contract TevaPaymasterV2 is IPaymaster, OwnableUpgradeable {
        ...
        returns (bytes4 magic, bytes memory context)
    {
        ...
        if (paymasterInputSelector == IPaymasterFlow.approvalBased.selector) {
            context = handleApprovalBasedFlow(_transaction);
    ...
    function handleApprovalBasedFlow(
        ...
        requiredToken = (requiredToken * (10 ** 4 + priceBufferBps)) / 10 ** 4;
        ...
        context = abi.encode(userAddress, true, token);
    }
    ...
    function postTransaction(
        ...
        // Only proceed with refunds if the transaction was successful
        require(_txResult == ExecutionResult.Success, "Transaction failed");

        // Decode the user address and token from the context
        (address userAddress, bool isApprivalBasedFlow, address token) = abi
            .decode(_context, (address, bool, address));

        // refund support only for approval-based flow
        if (!isApprivalBasedFlow) return;

        // Calculate the actual gas refunded in terms of ETH
        uint256 refundETHAmount = _maxRefundedGas * _transaction.maxFeePerGas;

        require(refundETHAmount > 0);
        ...
        // Calculate the required ERC20 tokens to be sent to the paymaster
        // (Equal to the value of refundETHAmount)
        uint256 requiredToken = (refundETHAmount * ethUsdPrice) /
            (tokenUsdPrice * (10 ** additionalDecimals));
        ...
        require(
            IERC20Upgradeable(token).transfer(userAddress, requiredToken),
