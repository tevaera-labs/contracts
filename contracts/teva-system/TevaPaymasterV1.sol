...
contract TevaPaymasterV1 is IPaymaster, OwnableUpgradeable {
    ...
    function handleApprovalBasedFlow(
        ...
        uint8 additionalDecimals = 18 - tokenDecimals; // additional decimal needed to make it 18
        ...
        uint256 requiredToken = (requiredEth * ethUsdPrice * priceBufferBps) /
            (tokenUsdPrice * (10 ** additionalDecimals) * (10 ** 4));
    ...
    function handleGeneralFlow(Transaction calldata _transaction) internal {
        ...
        bool isGaslessBuy = caller == tevaMarket &&
            ...
            txAmount > 100000000000000000; // txn with more than 0.1 ETH

        // if the user is trying to buy an asset worth more than 0.1 ETH, we will bear the gas fees.
        if (!isGaslessBuy && !eligibleGaslessWallets[userAddress]) {
